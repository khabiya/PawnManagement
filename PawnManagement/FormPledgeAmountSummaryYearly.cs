...
{
  public class FormPledgeAmountSummaryYearly : Form
  {
    ...
    public FormPledgeAmountSummaryYearly() => this.InitializeComponent();
    ...
    private DataTable billReport(int year)
    {
      ...
      if (this.cbShopCodes.Text == "")
        dataTable2 = SQLHelper.GetDataTable(string.Format("select tblDates.BDate, January, February, March, April, May, June, July, August, September, October, November, December from \r\n\r\n                                (((((((((((tblDates left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as January FROM tblPledge group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=1 and Year([BillDate])={0} order by Day([BillDate])) as tbl1 on tblDates.BDate = tbl1.BDate) \r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as February FROM tblPledge group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=2 and Year([BillDate])={0} order by Day([BillDate])) as tbl2 on tblDates.BDate = tbl2.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as March FROM tblPledge group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=3 and Year([BillDate])={0} order by Day([BillDate])) as tbl3 on tblDates.BDate = tbl3.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as April FROM tblPledge group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=4 and Year([BillDate])={0} order by Day([BillDate])) as tbl4 on tblDates.BDate = tbl4.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as May FROM tblPledge group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=5 and Year([BillDate])={0} order by Day([BillDate])) as tbl5 on tblDates.BDate = tbl5.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as June FROM tblPledge group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=6 and Year([BillDate])={0} order by Day([BillDate])) as tbl6 on tblDates.BDate = tbl6.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as July FROM tblPledge group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=7 and Year([BillDate])={0} order by Day([BillDate])) as tbl7 on tblDates.BDate = tbl7.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as August FROM tblPledge group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=8 and Year([BillDate])={0} order by Day([BillDate])) as tbl8 on tblDates.BDate = tbl8.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as September FROM tblPledge group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=9 and Year([BillDate])={0} order by Day([BillDate])) as tbl9 on tblDates.BDate = tbl9.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as October FROM tblPledge group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=10 and Year([BillDate])={0} order by Day([BillDate])) as tbl10 on tblDates.BDate = tbl10.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as November FROM tblPledge group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=11 and Year([BillDate])={0} order by Day([BillDate])) as tbl11 on tblDates.BDate = tbl11.BDate)\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as December FROM tblPledge group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=12 and Year([BillDate])={0} order by Day([BillDate])) as tbl12 on tblDates.BDate = tbl12.BDate", (object) year), ref strError);
      else
        dataTable2 = SQLHelper.GetDataTable(string.Format("select tblDates.BDate, January, February, March, April, May, June, July, August, September, October, November, December from \r\n\r\n                                (((((((((((tblDates left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as January FROM tblPledge {1}  group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=1 and Year([BillDate])={0} order by Day([BillDate])) as tbl1 on tblDates.BDate = tbl1.BDate) \r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as February FROM tblPledge {1}  group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=2 and Year([BillDate])={0} order by Day([BillDate])) as tbl2 on tblDates.BDate = tbl2.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as March FROM tblPledge {1}  group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=3 and Year([BillDate])={0} order by Day([BillDate])) as tbl3 on tblDates.BDate = tbl3.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as April FROM tblPledge {1}  group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=4 and Year([BillDate])={0} order by Day([BillDate])) as tbl4 on tblDates.BDate = tbl4.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as May FROM tblPledge  {1} group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=5 and Year([BillDate])={0} order by Day([BillDate])) as tbl5 on tblDates.BDate = tbl5.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as June FROM tblPledge  {1} group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=6 and Year([BillDate])={0} order by Day([BillDate])) as tbl6 on tblDates.BDate = tbl6.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as July FROM tblPledge {1}  group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=7 and Year([BillDate])={0} order by Day([BillDate])) as tbl7 on tblDates.BDate = tbl7.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as August FROM tblPledge  {1} group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=8 and Year([BillDate])={0} order by Day([BillDate])) as tbl8 on tblDates.BDate = tbl8.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as September FROM tblPledge {1}  group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=9 and Year([BillDate])={0} order by Day([BillDate])) as tbl9 on tblDates.BDate = tbl9.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as October FROM tblPledge {1}  group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=10 and Year([BillDate])={0} order by Day([BillDate])) as tbl10 on tblDates.BDate = tbl10.BDate)\r\n\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as November FROM tblPledge  {1} group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=11 and Year([BillDate])={0} order by Day([BillDate])) as tbl11 on tblDates.BDate = tbl11.BDate)\r\n                                left outer join\r\n\r\n                                (SELECT Day([BillDate]) AS BDate, Sum([Amount]) as December FROM tblPledge {1}  group by Day([BillDate]), Month([BillDate]), Year([BillDate]) having Month([BillDate])=12 and Year([BillDate])={0} order by Day([BillDate])) as tbl12 on tblDates.BDate = tbl12.BDate", (object) year, (object) " where shopcode = @ShopCode "), new List<OleDbParameter>()
        {
    ...
    private void FormPledgeAmountReport_Load(object sender, EventArgs e)
    {
      this.getShopCodes();
    ...
    private void viewFullScreenToolStripMenuItem_Click(object sender, EventArgs e)
    {
      if (!(sender is ToolStripItem toolStripItem) || !(toolStripItem.Owner is ContextMenuStrip owner))
        return;
      int num = (int) new FormDataGridView((DataTable) ((DataGridView) owner.SourceControl).DataSource, "Pledge  Amount Summary (yearly)").ShowDialog();
    }
    ...
    private void InitializeComponent()
    {
      ...
      this.dataGridView1.Size = new Size(1002, 565);
      ...
      this.cbYear.BackColor = Color.Azure;
      ...
      ((Control) this.headerPanel2).Location = new Point(627, 3);
      ...
      ((Control) this.headerPanel2).TabIndex = 84;
      ...
      ((Control) this.glassButton3).Location = new Point(-42, 513);
      ...
      ((Control) this.glassButton4).Location = new Point(92, 512);
      ...
      ((Control) this.headerPanel1).Location = new Point(405, 3);
      ((Control) this.headerPanel1).Name = "headerPanel1";
      this.headerPanel1.PanelIcon = (Icon) null;
      this.headerPanel1.PanelIconVisible = false;
      ((Control) this.headerPanel1).Size = new Size(208, 47);
      ((Control) this.headerPanel1).TabIndex = 83;
      ...
      ((Control) this.glassButton1).Location = new Point(-101, 513);
      ...
      ((Control) this.glassButton2).Location = new Point(33, 512);
      ...
      ((Control) this.headerPanel7).Location = new Point(56, 3);
      ...
      this.cbShopCodes.TextChanged += new EventHandler(this.cbYear_SelectedIndexChanged);
      ...
      ((Control) this.customPanel1).Size = new Size(1002, 55);
      ...
      this.Name = nameof (FormPledgeAmountSummaryYearly);
      this.Text = "FormPledgeAmountReport";
      this.Load += new EventHandler(this.FormPledgeAmountReport_Load);
